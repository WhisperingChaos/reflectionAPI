 ...
 * 
 */
CmmdElemHTTP.prototype = Object.create( CmmdElem.prototype)
...
Test.prototype.inform = function ( status, other) {
	var name = this.name == null ? this.constructor.name : this.name
	var mess = 'Inform: Test: \'' + name + '\' Status: \''+ status + '\'.'
...
Test.prototype.verify = function () {
	...
	if ( this.name == null){ this.name = this.constructor.name; }
	try {
	...
	deferred.resolve( '\'' + this.name + '\' Verify successful!')
	return promise
}
Test.prototype.execute = function () {
	...
	return this.verify()
		.then( function (){
			return this.inputContextChainForge()
		}.bind(this))
		.then( function ( contextInput){
...
Test_RESTget.prototype = Object.create( Test.prototype);
Test_RESTget.prototype.constructor = Test_RESTget
function Test_RESTget (){
	Test.call(this);
	this.inputContext( { "inp" : { "hostRestServer": { "IP_URLaddress" : '172.17.0.125', "port":'8080'}}})
	...
	this.command( new CmmdHTTPget().cmmdElemSpec( cmmdelem))
}
...
function Test_Version (){
	...
	this.cmmd.cmmdElemSpec( new CmmdElemLiteral('/version'))
...
function Test_ServiceList (){
	...
	this.cmmd.cmmdElemSpec( new CmmdElemLiteral('/q/listservices'))
...
var test_Version = new Test_Version()
test_Version.execute()

var test_ServiceList = new Test_ServiceList()
test_ServiceList.execute()
